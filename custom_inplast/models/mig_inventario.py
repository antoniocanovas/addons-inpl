    ...
    def _update_mig_inventario(self):
        ...
        for r in self:
            ...
            if product.id and location.id and not r.sml_id.id:
                ...
                lot = env['stock.lot'].search([('name', '=', nombrelote), ('product_id', '=', product.id)])
                if lot.ids:
                    lote = lot[0].id
                ...
                newsml = env['stock.move.line'].create({
                    'product_id': product.id, 'location_id': 14, 'location_dest_id': location.id, 'qty_done': r.qty,
                    'company_id': env.company.id, 'lot_id': lote, 'move_id': newsm.id, 'state': 'done'})
