...
class AnalyticDistribution(models.Model):
    ...
    analytic_line_ids = fields.One2many(
        "account.analytic.line", "analytic_distribution_id", string="Analytic lines"
    )
    ...
    def compute_distribution(self):
        ...
        incomelines = self.env["account.move.line"].search(
            [
                ("account_id", "in", self.income_account_ids.ids),
                ("date", ">=", datefrom),
                ("date", "<=", dateto),
            ]
        )
        ...
        expenselines = self.env["account.move.line"].search(
            [
                ("account_id", "in", self.expense_account_ids.ids),
                ("date", ">=", datefrom),
                ("date", "<=", dateto),
            ]
        )
        ...
        amount = income_credit - income_debit - expense_debit + expense_credit
